...
# Le spec n'est pas recopié dans le paquet : il vit dans le dépôt
# (`scripts/progedo_logit/feature_spec.json`). Ordre de recherche (cf. `resources`) :
#   1. la variable d'environnement, qui prime toujours ;
#   2. la racine désignée par MOBILITY_CORE_REPO_ROOT, puis la remontée depuis le
#      répertoire courant ;
#   3. le conteneur `controller`, où `scripts/` est monté sous /app/scripts.
...
# des scripts de mesure — cf. `mobility_core.residence_zone` et le docstring de
...
def residence_zone(lat: float | None, lon: float | None) -> str:
    ...
    **Qui classe quoi, désormais.** La couronne de RÉSIDENCE se lit sur le persona
    (trait `residence_zone`, `mobility_core.residence_zone`, ticket 021) ; le TEMPS
