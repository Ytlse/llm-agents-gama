...
//	map filtering <- map(["highway"::["primary", "secondary", "tertiary", "motorway", "living_street", "residential", "unclassified"], "building"::["no"]]);
	map filtering <- map(["highway"::["primary", "secondary", "motorway"], "building"::["yes"]]);
	//OSM file to load
	file<geometry> osmfile <- file<geometry> (osm_file("../includes/TulsaChicago.osm", filtering));
...
//{
//	parameter "File:" var: osmfile <- file<geometry> (osm_file("../includes/TulsaChicago.osm", filtering));
//	output
//	{
//		display map type: 3d
//		{
