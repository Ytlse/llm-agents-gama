...
# Ressource d'ACCÈS RESTREINT : présente dans le dépôt, jamais dans le paquet distribué —
# elle reproduit le plan de sondage de l'enquête (ticket 038). Résolue par
# `restricted_data_path`, qui regarde d'abord `MOBILITY_CORE_EMC2_DATA_DIR`.
TABLE_RESOURCE = "zf_couronne.json"

# Évalué à l'import, pour les appelants qui testent son existence. `load()` re-résout à
# chaque appel : une variable d'environnement posée après l'import reste prise en compte.
...
class CouronneTable:
    ...
    def load(cls, resource: Path | None = None) -> CouronneTable:
        ...
        if not path.exists():
            raise ResidenceZoneError(
                "table des couronnes absente : la couronne d'un domicile ne se devine pas.\n"
