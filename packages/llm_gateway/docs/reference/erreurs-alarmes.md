...
|---|---|---|
| **Aucun provider disponible** (`RuntimeError` de `select_provider`) | attente locale `resilience.provider_wait_seconds` (8 s), un essai toutes les `saturation_poll_seconds` (2 s) ; `saturation_retries` (2) `retry` Celery espacés de `saturation_retry_seconds` (12 s) | **éligibles** = l'instance épinglée, sinon les `instances_admises`, sinon toutes (une instance exclue, libre ou non, ne compte pas). Le lot **continue d'attendre** si une éligible est seulement **occupée** (fenêtre RPM/TPM pleine, lissage, concurrence ; `[ALARME] Providers occupés` une fois, `alarme:providers_occupes`) ou en refroidissement **rouvrant avant l'abandon du client** — toujours borné par l'attente du client (`resilience.client_wait_seconds` 120 s, ou `wait_timeout` de l'instance épinglée, moins `client_wait_margin_seconds` 10 s, mesurés depuis le premier essai du lot). Sinon, ou si `abandon_when_busy` : `[ALARME] Tous les providers LLM saturés ou indisponibles`, `alarme:providers_satures`, **échec** de jusqu'à 100 tâches, **qualifié** : `quota_journalier` si toutes les éligibles sont au quota du jour (reprise à la réinitialisation), `surcharge_fournisseur` sinon (reprise à la plus proche réouverture connue), aucun genre si aucune éligible n'est connue |
...
| `ProviderServerError` (5xx, troncature) | cooldown du provider **60 s**, `retry` Celery après `min(backoff_base_seconds × 2^tentative, 30 s)` tant que `retries < max_retries` (50) **et** que le client attend encore après ce délai | sinon échec `error_kind="surcharge_fournisseur"`, `resume_at` = fin du cooldown, `[ALARME] Surcharge fournisseur`, `alarme:surcharge_fournisseur` — rendu **avant** le « Timeout expiré » du client |
| `ProviderClientError` **429** (par minute) | cooldown = délai annoncé par le provider (`_parse_ratelimit_reset_seconds`, +2 s de marge, borné à [10, 3600] s, défaut 60), même backoff et `retry`, même borne par l'attente du client | sinon échec `surcharge_fournisseur`, `resume_at` = fin du cooldown, même alarme |
