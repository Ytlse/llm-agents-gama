...
class GoogleAdapter(BaseAdapter):
    ...
    def _generation_config(self, request: InternalRequest) -> dict[str, Any]:
        """Le `generationConfig` envoyé à l'API — extrait de `call` pour être testable tel quel.

        Il l'était par une COPIE dans les tests, qui ne contenait même pas le réglage de
        niveau : le défaut du 2026-09-11 est parti en production sans qu'un test le voie.
        Une méthode appelable ferme cette porte.

        **Profondeur de réflexion** (2026-09-10). Jusque-là aucun `thinkingConfig` n'était
        envoyé, donc tous les Gemini tournaient avec leur réflexion par défaut, non pilotée —
        alors que `thoughtsTokenCount` était déjà lu et compté. `None` conserve ce
        comportement.

        Le réglage vit **dans** `thinkingConfig`, jamais à côté. Le niveau y avait été posé en
        frère de `maxOutputTokens`, sur la foi de la documentation ; l'API répond alors 400
        « Unknown name "thinking_level" at generation_config » à CHAQUE appel, et l'expérience
        tombe en entier. Vérifié contre l'API le 2026-09-11 :

            generationConfig.thinking_level               → 400
            generationConfig.thinkingLevel                → 400
            generationConfig.thinkingConfig.thinkingLevel → 200

        Niveau et budget sont exclusifs, et l'API le dit aussi (« You can only set only one of
        thinking budget and thinking level »). La cascade `resolve_inference` les refuse déjà
        ensemble ; le `elif` ci-dessous est la seconde barrière.
        """
        ...
        if budget is not None and budget != 0:
            # La pensée consomme le budget de SORTIE : sans réserve, un budget généreux fait
            # tronquer la réponse (MAX_TOKENS) et l'appel est perdu.
