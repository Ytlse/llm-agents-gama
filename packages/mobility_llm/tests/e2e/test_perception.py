"""
tests/e2e/test_perception.py — la catégorie perception_filter, de bout en bout, sur un gateway réel.

Exige un gateway en marche et des clés de provider : ``LLM_GATEWAY_E2E_URL=http://localhost:8000``.
Sans la variable, le module est sauté (marqueur e2e). Reste exécutable en script :
...
if not E2E_URL and "pytest" in sys.modules and __name__ != "__main__":
    pytest.skip("LLM_GATEWAY_E2E_URL non défini : pas de gateway réel", allow_module_level=True)
...
def main(base_url: str) -> None:
    """Lève AssertionError en cas d'échec (pytest) ; le script traduit en code retour."""
    print(f"\n{'═' * 60}")
    ...
    with httpx.Client(timeout=30) as client:
        ...
        if resp.status_code != 202:
            ...
            raise AssertionError('échec du scénario perception_filter')
