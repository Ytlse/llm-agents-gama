...
**Licence déclarée** (ticket 038) : `Apache-2.0`, avec `LICENSE` et `NOTICE` embarqués. Sans
réserve — les prompts et gabarits citent des communes et des noms de réseaux, ils ne
reproduisent aucune donnée de l'enquête EMC². Les réserves sur l'enquête vivent dans le
`NOTICE` de `mobility-core`, dont ce paquet dépend.
