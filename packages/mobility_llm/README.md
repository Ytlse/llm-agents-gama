...
- `prompts.yaml` — `active: {itinary_multi_agent: expert_m4}` et `prompts: {clé:
  {content: …}}` ; dix-huit variantes (`expert_m4` et sa lignée `expert_chaine_m5`…`m7.1`,
  `expert_m1`, `prompt_minimal`, `b_min`, `expert_gem_3.8_v2`, plus les archivées `expert`,
  `b0_pristine`, `persona_v1`…`v5`, `minimal_persona`, `expert_gem_3.8_v1`).
  Une entrée portant `_archive: {statut: archive}` n'est plus proposée par le tableau de bord
  mais **reste servable** : c'est un retrait du choix, pas un refus de service. Le refus de
  service, lui, vient de `_invalidation` ou d'un `_neutralite` non conforme ou périmé.
