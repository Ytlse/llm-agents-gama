"""
Ce fichier sert d'interface en ligne de commande (CLI) pour tester et déboguer
les fonctionnalités du scénario V1 de manière isolée.

Il permet de lancer des fonctions spécifiques de l'agent cognitif, comme la planification
de trajets (test_plan) ou le mécanisme de réflexion sur la mémoire (test_reflection),
sans avoir à lancer une simulation complète avec GAMA.
"""
import sys
import os
...
async def test_plan():
    ...
    await agent.evaluate_and_choose_travel_plan(
        Context(
            person=person,
            timestamp=timestamp,
        ),
...
async def test_reflection():
    ...
    await agent.reflect_on_short_term_memory(Context(
        person=person,
