...
def _racine_donnees() -> str:
    """Ancêtre portant `data/weather`, vu de l'hôte comme du conteneur.

    Un nombre de `..` fixe ne peut pas convenir des deux côtés : ce module vit sous
    `<dépôt>/services/llm-agents/urban_mobility_agents/utils/` sur l'hôte mais sous
    `/app/urban_mobility_agents/utils/` dans le conteneur, où `./data/weather` est monté
    sur `/app/data/weather`. `../../../` désignait la racine avant le ticket 039 ; il
    désigne `services/` depuis, et les bulletins devenaient introuvables — un défaut MUET,
    `get_weather()` rendant `None` et les tests se contentant de s'ignorer.
    """
    # On cherche le FICHIER, pas le dossier : sur l'hôte `services/llm-agents/data/weather`
    # existe mais vide — c'est le point de montage du conteneur. S'arrêter là rendrait une
    # racine plausible et des bulletins introuvables.
    ...
    while True:
        ...
        if parent == ici:
            # Rien trouvé : on rend l'ancien calcul pour que l'erreur nomme un chemin.
