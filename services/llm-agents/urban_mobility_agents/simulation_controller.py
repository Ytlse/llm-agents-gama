...
def _build_anticipation(
    ...
    système (variante `expert_m4`), et l'information de disponibilité reste
...
class SimulationLoopV1(BaseScenario):
    ...
    def __init__(
        ...
        # Plancher journalier (ticket 048) : jour simulé (ordinal) où le plancher a
        # déjà soumis une réflexion pour cet agent. Il ne part qu'une fois par jour.
    ...
    def _try_schedule_next_after(
        ...
        # Chaîne CYCLIQUE, implémentation unique partagée avec la plateforme d'expériences
        # (`chaine_activites`) : c'est la divergence entre les deux qui faisait manquer à la
        # plateforme le retour au domicile de chaque journée (ticket 045, A1).
    ...
    def _refill_precomputed_queue(self, person: Person) -> None:
        ...
        if horizon_act is None or horizon_ts is None:
            return
        # Même chaîne cyclique partagée que partout ailleurs (ticket 045, A1) : elle porte
        # le garde `len <= 1`, l'appariement par `id` et le contrôle des deux localisations.
    ...
    async def sync(
        ...
        if (
            ...
            # Plancher journalier (ticket 048) : passé l'heure dite, tout agent dont le
            # tampon n'est pas vide devient éligible, quel que soit son remplissage, et
            # au plus une fois par jour simulé. Le seuil volumétrique n'est pas touché :
            # le plancher s'y AJOUTE, il ne le remplace pas.
            _floor_day = None
        ...
        while person_last_planned_act:
            ...
            for person, act_N in eligible:
                ...
                if last_act is None or last_ts is None:
                    continue
                # Même chaîne cyclique partagée que partout ailleurs (ticket 045, A1). Elle
                # ajoute ici le contrôle des localisations que ce site ne faisait pas : une
                # extrémité sans localisation ne produit aucun trajet calculable, et la
                # personne sort du suivi au lieu d'entrer dans la vague pour y échouer.
