...
def _racine_partagee(*repere: str) -> Path:
    """Ancêtre de `base_dir` sous lequel `repere` existe, vu de l'hôte comme du conteneur.

    Même raison que `_candidats_providers_yaml` : `base_dir` vaut `<dépôt>/services/llm-agents`
    sur l'hôte mais `/app` dans le conteneur, où les répertoires du dépôt sont montés à côté
    du code. Un `..` fixe ne peut pas tomber juste des deux côtés — le nombre de crans diffère.
    On cherche donc le repère au lieu de le compter (ticket 039 : `llm-agents` est descendu
    d'un cran sur l'hôte, pas dans le conteneur).

    Rend le premier ancêtre qui contient `repere`, et à défaut le parent immédiat — l'ancien
    comportement, pour qu'un repère absent (dépôt partiel, test) ne lève pas ici mais échoue
    plus tard là où le fichier est vraiment lu.
    """
...
def _resolve_experiments_dir(package_dir: Path) -> Path:
    ...
    distinguer les deux cas fabriquait un `services/llm-agents/experiments/` parallèle,
    dont les chemins ne se résolvaient pas depuis `services/GAMA/CityTransport/`.
...
def _run_artifacts_disabled() -> bool:
    ...
    repointait le symlink `services/GAMA/CityTransport/results` — volant sa sortie à la
...
class GTFSConfig(BaseSettings, WorkdirPathResolutionMixin):
    ...
    #
    # ⚠ INVARIANT : le fichier vit dans `services/GAMA/CityTransport/includes/`, à côté de
    # `routes.shp` et `trip_info.json` dont il note l'empreinte pour prouver qu'il vient
    # de la même génération. Le conteneur `controller` monte `./services/GAMA` sur
    # `/services/GAMA` ; sur l'hôte le même répertoire est à `<dépôt>/services/GAMA`. Les deux
    # se trouvent en cherchant le repère, pas en comptant les crans (cf. `_racine_partagee`).
    # Le déplacer hors de ce répertoire casserait le contrôle de fraîcheur, qui cherche ses
    ...
    # `services/llm-agents/data/…` invisible du peupleur : c'est exactement ce qui s'est produit du
...
class AgentConfig(BaseSettings, WorkdirPathResolutionMixin):
    ...
    # Plancher journalier de consolidation (ticket 048). Le seuil ci-dessus est
    # VOLUMÉTRIQUE : le moment où un agent consolide est donc une fonction du nombre
    # de déplacements qu'il fait ce jour-là — un agent à un seul déplacement produit
    # ~6 entrées et ne consolide pas, un agent à quatre consolide deux fois. La
    # position d'un choc par rapport à la consolidation devient une variable non
    # contrôlée de l'expérience, et comme la décision ne lit QUE la mémoire longue,
    # un souvenir non consolidé n'existe pas pour elle. Le plancher rend le calendrier
    # déterministe : à l'heure dite, tout agent dont le tampon n'est pas vide soumet
    # une réflexion, une fois par jour simulé au plus, quel que soit son remplissage.
    # 22 h laisse la nuit simulée au drainage EDF, conformément au ticket 010.
    ...
    # (`long_term_retrieval__time_decay = 0.7`), qui ne se lisait pas : 2,8 jours
    # reproduit EXACTEMENT cette décroissance (0,7^Δt = exp(-Δt/2,8034), demi-vie
    # 1,94 jour). Un paramètre en jours se discute — « un souvenir ordinaire dure
    # trois fois plus longtemps » — là où une base d'exponentielle ne se discute pas.
    # C'est ce paramètre que les bras de sensibilité des expériences doivent viser :
...
class FactorySettings:
    ...
    def get(cls) -> Settings:
        ...
        # éditer directement services/llm-agents/config/config.yaml.
    ...
    def claim_run(cls) -> Settings:
        ...
        **À n'appeler que par le processus propriétaire du run** — le contrôleur, via
        ...
        if gama_results_link.parent.exists():
            ...
            # ./experiments sur /experiments). Sa cible doit donc s'exprimer dans la
            # disposition du dépôt, pas dans celle du contrôleur : depuis
            # services/GAMA/CityTransport, TROIS niveaux au-dessus donnent la racine,
            # puis experiments/. C'est pour que ce compte soit le même des deux côtés que
            # le montage porte le préfixe `services/` (ticket 039). Un calcul par
