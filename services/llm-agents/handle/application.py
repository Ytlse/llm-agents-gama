...
async def startup_event():
    ...
    """
    # Ticket 075 — la restauration se fait ICI, au tout début : l'index vectoriel de la mémoire
    # est ouvert à la construction de l'agent (au `/init`), et remplacer ses fichiers sous lui
    # laisserait une base ouverte sur des octets qui n'existent plus. Sans reprise demandée,
    # cet appel ne fait rien.
    def _empreinte_choc_courante() -> str:
        ...
        except Exception:  # noqa: BLE001 — l'identité ne doit jamais empêcher un démarrage
            return ""

    # Ticket 091 — par défaut on ne réutilise RIEN. La reprise se demande en nommant le run, et
    # n'a lieu que si l'identité du run nommé décrit la même expérience que la configuration
    # courante. Une identité absente, illisible ou différente fait échouer le démarrage en
    # nommant ce qui cloche : réutiliser la mémoire d'une autre expérience produirait une mesure
    ...
    # Ticket 090 — la trace de décisions est OUVERTE sur tout run, pas seulement sur une reprise :
    # c'est pendant la vie normale qu'elle s'écrit. Ne l'ouvrir qu'à la reprise la laissait vide,
    # donc inerte — défaut constaté le 2026-09-17 sur le run 2026-09-17_06_26, zéro ligne tracée
    ...
    # sans cela elle deviendrait un cache que personne n'a demandé.
    if _reprise:
...
async def init(request: WorldInitRequest):
    ...
    # Ticket 075 — une simulation qui (re)démarre rejoue depuis son t0. Si un point de
    # reprise existe, la mémoire sur disque lui est POSTÉRIEURE : elle appartient à un run
    # interrompu, et les jours rejoués la réécriraient. On la ramène au point, et on gèle
    # jusqu'à ce que l'horloge simulée le dépasse. Sur un run neuf, il n'y a pas de point et
    # cet appel ne fait rien. C'est ici et pas au démarrage du contrôleur : `CONT=1` ne
    # redémarre que GAMA, le conteneur `controller`, lui, ne repasse pas par son startup.
    restaurer_si_demande(settings.workdir, reprise_demandee=True)
...
async def poser_accident(raw: Request):
    """`{lat, lon, debut_ts, duree_minutes}` → l'accident posé, ou un refus motivé."""
    from trip_helper import accidents as accidents_module

    # Corps lu en brut (client HTTP GAMA / h2c), comme /sync et /calibrate.
    body = await raw.body()
    if not body:
    ...
    if registre is None:
        # Refus explicite plutôt que silence : sans lui, l'expérimentateur croirait avoir
        # posé un accident dans un run où le régime est décoché.
        logger.error(
