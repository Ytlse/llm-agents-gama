...
def _population():
    """3 personnes : 3, 2 et 1 activités → 3 + 2 + 0 = 5 déplacements attendus (J2).

    La journée se REFERME (ticket 045, A1) : n activités localisées valent n déplacements, le
    dernier ramenant à la première activité — le retour au domicile est un déplacement comme un
    autre. p1, seule à son unique activité, n'en a toujours aucun : une chaîne d'un seul maillon
    ne boucle pas sur elle-même.

    Les deux fermetures de ce banc ne se ressemblent pas, et c'est ce qui les rend utiles :
    - p3 (a2 → a0) va de HOME à HOME : comptée dans les attendus BRUTS, mais classée inexploitable
      au motif `origine_egale_destination` (« même lieu 1 » dans le journal de préparation) ;
    - p2 (b1 → b0) va du GYM à HOME : un déplacement réel, calculé comme les autres.
    """
    return [
...
def test_J2_deplacements_derives(pop_dir):
    ...
    # 3 + 2 + 0 : chaque personne d'au moins deux activités rend AUTANT de déplacements que
    # d'activités, fermeture de la journée comprise (p3 : a0→a1, a1→a2, a2→a0 ; p2 : b0→b1, b1→b0).
...
def test_J6_consulter_une_personne(tmp_path, pop_dir):
    ...
    # Trois lignes depuis la fermeture de la journée, triées par heure de départ : le retour a2 → a0
    # part à l'heure programmée de a0 (23 h 45 sur ce banc : faute de `scheduled_start_time` sur la
    # première activité, le chargeur prend son `start_time` moins 15 min), donc en dernier.
    assert [l.activity_id for l in lignes] == ["a1", "a2", "a0"]
    # Les deux vrais trajets portent les 5 propositions du moteur ; la fermeture HOME → HOME n'en
    # a aucune — aucun moteur n'est appelé pour aller d'un lieu à lui-même.
...
def test_J7_J8_completude(tmp_path, pop_dir):
    ...
    jeu = J.Jeu.charger(tmp_path / "jeu")
    c = jeu.couverture()
    # 5 attendus : p3/a1 et p3/a2 couverts, p2/b0 (GYM → HOME, la fermeture de p2) couvert aussi ;
    # p2/b1 sans proposition (le moteur rend vide pour le GYM) et p3/a0 inexploitable (même lieu).
    # Le taux se mesure sur les EXPLOITABLES (4), pas sur les attendus bruts (5) : exiger
    # qu'une fermeture sur place soit couverte rendrait tout jeu incomplet à jamais.
    ...
    # Incomplet pour une VRAIE raison : p2/b1 n'a aucune proposition alors qu'il en attendait.
...
def test_J11_reprise(tmp_path, pop_dir):
    ...
    # 5 attendus : a1, a2 et b0 calculés, a0 écrit sans appel de moteur (même lieu) → 4 ;
    # seul b1, qui vise le GYM, tombe sur le moteur cassé.
    ...
    # Plus rien à recalculer, et le jeu est COMPLET : les 4 déplacements qui pouvaient être
    # couverts le sont. Le cinquième attendu est la boucle HOME → HOME de p3, qui n'a pas
    # d'itinéraire à calculer — l'exiger rendrait la complétude inatteignable pour toute
    # population dont les journées reviennent au domicile, c'est-à-dire toutes.
...
def test_J15_journal_et_alarme(tmp_path, pop_dir):
    ...
    # 5 déplacements écrits : 3 sans proposition (WORK vide, GYM vide, et la boucle même lieu).
...
def test_D18_comparer_offre_jour(tmp_path, pop_dir):
    ...
    # 4 déplacements portent une offre TC : les 5 attendus moins la boucle HOME → HOME, sans propositions.
