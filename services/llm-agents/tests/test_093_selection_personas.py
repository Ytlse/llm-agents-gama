...
def _trajet(personne, mode, options, *, heure="08:00:00", jour="2026-03-16", activite="a1"):
    return {
...
def _journee_variee(personne: str, *, trajets=4, options=5, modes=("Voiture Privée", "Marche")):
    """Une journée qui satisfait le critère : assez de trajets, des options, plusieurs modes."""
    return [
        _trajet(personne, modes[i % len(modes)], options,
                heure=f"{6 + i:02d}:00:00", activite=f"a{i}")
...
class TestCritere:
    ...
    def test_A2_un_seul_trajet_mono_option_recale_quel_que_soit_le_reste(self, tmp_path):
    ...
    def test_A3_un_seul_mode_recale_meme_avec_des_options(self, tmp_path):
        run = _ecrire_run(tmp_path / "run",
                          _journee_variee("609", trajets=8, options=3,
                                          modes=("Voiture Privée",)))
    ...
    def test_A4bis_le_critere_ne_juge_pas_les_modes_proposes_mais_les_modes_CHOISIS(self, tmp_path):
        ...
        """
        run = _ecrire_run(tmp_path / "run",
                          _journee_variee("41275", trajets=6, options=9,
                                          modes=("Voiture Privée",)))
        assert not S.est_candidat(S.mesurer(run)["41275"])

    @pytest.mark.skipif(not (RUN_REFERENCE / "moves.csv").is_file(),
                        reason="run de référence absent (data/experiences n'est pas versionné)")
...
class TestChoix:
    ...
    def test_A6_les_conserves_sont_retenus_de_droit_et_leur_critere_est_ecrit(self, tmp_path):
        ...
        retenus = S.choisir(S.mesurer(run), json.loads(source.read_text()),
                            conserver=["899549", "616478"], combien=10)
        par_id = {r.person_id: r for r in retenus}
        assert set(["899549", "616478"]) <= set(par_id)
    ...
    def test_A7_dix_agents_et_les_modes_dominants_couverts(self, tmp_path):
        ...
        run, source = self._fixture(tmp_path,
                                    list(modes) + [str(2000 + i) for i in range(10)],
                                    modes_par_agent=modes)
        retenus = S.choisir(S.mesurer(run), json.loads(source.read_text()),
                            conserver=[], combien=10)
        assert len(retenus) == 10
        dominants = {r.mode_dominant for r in retenus}
        assert {"car", "walking", "public_transport", "cycling"} <= dominants

    def test_A8_deux_executions_rendent_la_meme_liste_dans_le_meme_ordre(self, tmp_path):
        run, source = self._fixture(tmp_path, [str(3000 + i) for i in range(14)])
        population = json.loads(source.read_text())
        mesures = S.mesurer(run)
        premier = [r.person_id for r in S.choisir(mesures, population, conserver=[], combien=10)]
        second = [r.person_id for r in S.choisir(mesures, population, conserver=[], combien=10)]
    ...
    def test_A12_a_mode_dominant_egal_le_plus_observable_passe_avant(self, tmp_path):
        ...
        lignes += _journee_variee("9002", trajets=12,
                                  modes=("Voiture Privée", "Marche", "Vélo"))  # riche
        run = _ecrire_run(tmp_path / "run", lignes)
    ...
    def test_A10_moins_de_candidats_que_demande_refuse_au_lieu_de_completer(self, tmp_path):
        run, source = self._fixture(tmp_path, [str(5000 + i) for i in range(4)])
        with pytest.raises(SystemExit) as refus:
            S.choisir(S.mesurer(run), json.loads(source.read_text()),
                      conserver=[], combien=10)
        assert "4" in str(refus.value) and "10" in str(refus.value)

    def test_A10bis_un_conserve_absent_de_la_source_refuse_en_le_nommant(self, tmp_path):
        run, source = self._fixture(tmp_path, [str(6000 + i) for i in range(12)])
        with pytest.raises(SystemExit) as refus:
            S.choisir(S.mesurer(run), json.loads(source.read_text()),
                      conserver=["999999"], combien=10)
        assert "999999" in str(refus.value)


class TestManifeste:
    def test_A11_le_manifeste_porte_le_critere_le_run_et_les_chiffres_mesures(self, tmp_path):
        lignes = []
        for personne in [str(7000 + i) for i in range(12)]:
            lignes += _journee_variee(personne, trajets=6)
        run = _ecrire_run(tmp_path / "run", lignes)
        source = _ecrire_source(tmp_path / "pop.json", [str(7000 + i) for i in range(12)])
    ...
    def test_A11bis_la_population_ecrite_ne_contient_que_des_agents_de_la_source(self, tmp_path):
...
class TestVerification:
    ...
    def test_une_population_modifiee_a_la_main_est_detectee(self, tmp_path):
        ...
        (sortie / "population.json").write_text(json.dumps(agents[:9]), encoding="utf-8")
