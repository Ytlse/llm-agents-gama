...
def test_S2_non_couvert_trace_et_hors_parts(banc, tmp_path):
    ...
    # 5 attendus depuis que la journée se referme (ticket 045). Deux en sortent : p2/b1, retiré du
    # jeu ci-dessus, est NON COUVERT (il reste au dénominateur) ; p3/a0, la boucle HOME → HOME, est
    # INEXPLOITABLE (il en sort). Restent 3 décidés sur 4 exploitables.
    assert (
...
def test_S7_meme_lecteur_et_validation(banc):
    ...
    # 5 traces archivées : les 4 déplacements décidés (p3 a1/a2, p2 b1/b0) plus la fermeture
    # HOME → HOME de p3, archivée « inexploitable » pour qu'une reprise ne la retente pas.
    assert len(relu.decisions) == 5 and (exe.dossier / "moves.csv").exists()
    import csv

    with open(exe.dossier / "moves.csv", encoding="utf-8") as f:
        lignes = list(csv.DictReader(f))
    # moves.csv ne porte que les déplacements réellement décidés : 4 lignes, pas 5.
...
def test_S9_Q5_reprise_sans_double_depense(banc):
    ...
    # Une seule décision était archivée avant la pause : elle est RESSERVIE, jamais repayée.
    # Sur les 4 déplacements restants de la journée refermée, le décideur n'est sollicité que
    # deux fois : p3/a2 (retour maison) est un choix unique et p3/a0 (HOME → HOME) inexploitable.
    assert (
        dec2.appels == 2
        and c2["resservies"] == 1
    ...
    # Q6 : résultat final identique à l'exécution ininterrompue — les 5 traces, y compris la
    # fermeture inexploitable de p3, qui n'a aucune option retenue (`retenue` vaut None).
...
def test_S10_Q12_decideur_local_sans_quota(banc):
    ...
    # 4 décidés (p3 a1/a2, p2 b1/b0) mais 3 sollicitations : le retour p3/a2 est imposé par la
    # chaîne de véhicule (choix unique), et la fermeture p3/a0 n'atteint jamais le décideur.
    assert (
        c["sollicitations"] == 3
        and c["decides"] == 4
...
def test_Q9_derniere_ligne_tronquee(banc):
    exp = _exp(banc)
    exe = _execution(banc, exp)
    # les trois traces de p3 archivées (a1, a2 et la fermeture inexploitable a0) puis une ligne
    # à moitié écrite : la troncature porte donc sur la ligne 4
    _, ref = _lancer(banc, _exp(banc, nom="ref"))
    ...
    # Restent les deux déplacements de p2 (b0→b1 et la fermeture b1→b0), tous deux sollicités.
...
def test_E5_estimation_cite_ses_sources(banc):
    ...
    # L'estimation compte les déplacements COUVERTS du jeu : 5 attendus moins la fermeture
    # HOME → HOME, qu'aucun moteur ne peut proposer → 4 sollicitations, 4 × 750 jetons d'entrée.
    assert (
...
def test_E13_comparabilite(banc):
    ...
    # `attendus` est le dénominateur EXPLOITABLE : 5 attendus bruts moins la fermeture HOME → HOME.
    assert (
...
def test_E14_E16_synthese_couverture_et_rendu(banc):
    ...
    # Une seule décision sur les 5 attendus : la pause tombe à la 1re sollicitation, donc la
    # fermeture inexploitable de p3 n'a pas encore été archivée et rien n'est encore exclu.
...
def test_E18_sources_d_alea_et_nouvelle_execution(banc):
    ...
    # Les 5 traces, fermeture inexploitable comprise : elle n'a pas d'option retenue (None), et
    # le rejeu doit la rendre à l'identique plutôt que de la faire disparaître.
    assert {
        (t["person_id"], t["activity_id"], (t["retenue"] or {}).get("code"))
...
def test_E22_attendus_derives(banc):
    """Les attendus viennent de l'agenda, jamais d'une saisie — et la journée se REFERME.

    n activités localisées valent n déplacements, le dernier ramenant à la première activité
    (ticket 045, A1) : 3 + 2 + 0 = 5 sur ce banc, là où l'énumération par paires consécutives
    n'en voyait que 3 et laissait le retour au domicile jamais décidé.
    """
...
def test_D3_inexploitables_exclus_des_attendus(banc, tmp_path):
    ...
    try:
        ...
        logger.remove(sink)
    # Deux inexploitables désormais : p2/b1 rendu vide ci-dessus, et la fermeture HOME → HOME de
    # p3 que la journée refermée ajoute — 5 attendus bruts, 3 exploitables, 3 décidés.
    assert (
        c["inexploitables"] == 2
        and c["attendus"] == 5
...
def test_S6_progression_json_pour_le_tableau_de_bord(banc):
    ...
    # Numérateur et dénominateur comptent la MÊME chose (R9) : `attendus` est le décompte
    # EXPLOITABLE (4), dérivé du jeu à l'ouverture, et `faits` les déplacements réglés par le
    # runner (4). Un run terminé affiche donc 100 %. Avec le décompte brut au dénominateur,
    # toute exécution complète serait restée à 80 % dans le tableau de bord.
    assert (
        p["faits"] == 4
        and p["attendus"] == 4
...
class OccupeeNfois(DecideurDureeMinimale):
    ...
    seule proposition → `choix_unique`, décideur NON sollicité ; et `p3/a0`, la fermeture
    HOME → HOME de la journée, est inexploitable, donc jamais soumise non plus. Seuls 3 des 5
    déplacements passent par le décideur : ce décideur produit `3 × n` tentatives ratées.
    """
...
def test_R2_occupee_nest_pas_epuisee(banc, monkeypatch):
    ...
    )  # 2 échecs × 3 déplacements sollicités
...
def test_R3_commande_erreurs_detecte_trou(banc):
    ...
    exe.fermer()
    diag = ERR.diagnostiquer(exe.dossier)
    # 5 déplacements au jeu, 2 archivés : manquent p3/a1, la fermeture p3/a0 et la fermeture
    # p2/b0. Un seul est un TROU de chaîne (a1, suivi d'un a2 décidé) ; les deux fermetures
    # manquent en fin de journée, donc sans décision archivée après elles.
    assert (
        diag["nb_manquants"] == 3
