...
appris entre dans l'appel **comme une entrée de plus**. Pas de passe séparée, pas de prompt
...
- **G1 — un récit n'est jamais servi deux fois au même receveur.** Repère `foyer_lu_jusqu_a`,
  par receveur : ce que A produit après le passage de B n'est pas perdu, il est lu la nuit
  suivante. Le partage est au fil de l'eau, avec un décalage d'au plus une nuit, jamais une
...
class EtatFoyer:
    ...
    # G1 — horodatage simulé de la dernière consolidation où ce receveur a entendu son foyer.
...
def charger_etat(brut: dict | None) -> None:
    ...
    try:
        ...
        logger.info(
            f"[foyer] état relu du point de reprise : {len(_etat.lu_jusqu_a)} receveur(s) "
            f"avec un repère, {len(_etat.croyances_montrees)} croyance(s) déjà montrée(s)"
        )
...
def recit_du_soir(long_term_memory, receveur_id: str, maintenant: datetime) -> list[str]:
    ...
    Rend une liste d'énoncés déjà formatés, prêts à entrer dans l'appel. Vide quand le foyer
    est vide, quand le drapeau est éteint, ou quand personne n'a rien produit de neuf.

    G1 est structurel : le repère avance à la fin, et une réflexion déjà servie ne repart pas.
    Et un récit n'est JAMAIS copié dans la mémoire du receveur — il n'existe que dans l'appel,
    donc il n'y a rien à re-raconter. C'est ce qui borne la circulation à un saut sans aucun
    champ de plus.
    """
    if not actif():
        return []
    ...
    repere_brut = _etat.lu_jusqu_a.get(str(receveur_id))
    repere = None
    if repere_brut:
        try:
            repere = datetime.fromisoformat(repere_brut)
        except (TypeError, ValueError):
            repere = None

    borne = int(getattr(settings.agent, "memoire__recit_soir_max", 8))
    lignes: list[str] = []
    vus = 0
    for membre in sorted(autres, key=lambda m: m.person_id):
        reflexions = _reflexions_depuis(long_term_memory, membre.person_id, repere)
        vus += len(reflexions)
        for entree in reflexions:
            if len(lignes) >= borne:
                continue
            # ⚠ Le texte est CITÉ, pas reformulé. « Aucune nouvelle information » (auteur,
            # 2026-09-22) : ce qui circule est ce que son porteur a lui-même écrit, dans ses
            # mots. Le réécrire y ferait entrer une information qu'il n'a pas dite.
            lignes.append(
                f'- {membre.libelle} told you about their day: "{(entree.content or "").strip()}"'
            )

    if vus > len(lignes):
        # ⚠ Une troncature se COMPTE et s'ALARME. Elle n'est pas une politique de rationnement :
        # avec un bilan par membre et par nuit, le volume attendu est petit. Qu'elle se déclenche
        # signale que quelque chose ne va pas ailleurs.
        logger.error(
            f"[ALARME] [foyer] récit du soir TRONQUÉ pour {receveur_id} : {vus} bilan(s) "
            f"disponible(s), {len(lignes)} servi(s) (borne "
            f"`memoire__recit_soir_max` = {borne}). Avec un bilan par membre et par nuit, "
            f"cette borne ne devrait pas mordre — vérifiez le repère de lecture."
        )
        _compteurs_soir["troncatures"] += vus - len(lignes)

    if lignes:
        _etat.lu_jusqu_a[str(receveur_id)] = maintenant.isoformat()
    return lignes
...
def journaliser_compteurs() -> None:
    ...
    logger.info(
        ...
        f"croyance(s) transmis, {c['troncatures']} troncature(s). Concepts examinés : "
...
def bloc_du_soir(long_term_memory, receveur, maintenant: datetime) -> str:
    ...
    d'avoir écouté.
    """
    recits = recit_du_soir(long_term_memory, str(receveur.person_id), maintenant)
