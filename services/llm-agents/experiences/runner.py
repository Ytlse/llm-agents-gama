...
async def executer(
    ...
    *,
    ...
    # R9 (alerte A3) : le dénominateur se DÉRIVE DU JEU à l'ouverture, il ne s'accumule pas
    # au fil des décisions. Sans cela, une exécution interrompue n'annonce pas le même nombre
    # d'inexploitables qu'une exécution complète sur le même couple (population, jeu), et deux
    # colonnes cessent d'être comparables terme à terme.
    inexploitables_connus = jeu.inexploitables(attendus)
    attendus_exploitables_ouverture = len(attendus) - len(inexploitables_connus)
    logger.info(
        f"[execution] dénominateur arrêté à l'ouverture : {attendus_exploitables_ouverture} "
        f"déplacements exploitables sur {len(attendus)} attendus "
        f"({len(inexploitables_connus)} inexploitables dérivés du jeu {jeu.nom!r})"
    )
    prog = Progression(
    ...
    # Le dénominateur arrêté à l'ouverture fait foi (R9). S'il diverge de ce que l'exécution a
    # observé, c'est que le jeu a changé sous les pieds de l'exécution, ou qu'un déplacement a
    # été classé inexploitable pour une raison que le jeu ne portait pas : dans les deux cas la
    # mesure n'est pas celle qu'on croit, et cela doit se voir.
    if attendus_exploitables != attendus_exploitables_ouverture:
        logger.error(
            f"[ALARME] dénominateur incohérent : {attendus_exploitables_ouverture} exploitables "
            f"dérivés du jeu à l'ouverture, {attendus_exploitables} observés à la clôture "
            f"({len(inexploitables)} inexploitables archivés contre {len(inexploitables_connus)} "
            f"attendus). La couverture publiée retient le dénominateur d'ouverture."
        )
