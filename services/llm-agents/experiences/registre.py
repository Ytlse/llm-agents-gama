            ...
            lignes.append(
                {
                    **base,
                    ...
                    # Ticket 047 — le compte de choix forcés voyage avec les parts et avec
                    # le composite, systématiquement. DEUX comptes, parce qu'il y a deux
                    # périmètres et qu'ils diffèrent : celui de l'exécution accompagne la
                    # couverture et les parts, celui du score accompagne le composite. Un
                    # seul chiffre pour les deux se lirait de travers la moitié du temps.
...
def formater_table(lignes: list[dict], colonnes: list[str] | None = None) -> str:
    colonnes = colonnes or [
        ...
        "choix_forces",
        "composite_emd",
        # Ticket 047 — la seconde lecture est affichée PAR DÉFAUT, à côté de la première.
        # Reléguée derrière une option, elle ne serait pas lue, et c'est justement la
        # colonne qui dit si le composite note un décideur ou une offre.
    ...
    def cell(l, c):
        ...
        if c == "part_forces" and isinstance(v, float):
            return f"{100 * v:.1f} %".replace(".", ",")
        # Compte de choix forcés : « — » si l'exécution n'a pas de synthèse lisible. Un 0
        # affiché à sa place ferait croire qu'aucune décision n'était contrainte.
...
def synthese(dossier_execution: str | Path, referentiel: Path | None = None) -> dict:
    ...
    # A5 — les choix forcés entrent dans les parts, et leur NOMBRE dépend du bras. Un trajet
    # à itinéraire unique n'a été décidé par personne : il découle d'un choix antérieur du
    # même bras (partir en voiture le matin, c'est rentrer en voiture le soir). Mesuré sur les
    # 23 exécutions complètes de la v1 : 393 pour `duree_minimale`, 286 pour `aleatoire`.
    # On ne les retire PAS de la lecture principale — la contrainte est le cadre, pas le
    # défaut, et une vraie journée en comporte — mais on publie les deux lectures, parce qu'un
    # seul chiffre mélange ce que le décideur a décidé et ce que la situation lui imposait.
    ...
    return {
        ...
        # Seconde lecture (R10) : ce que le décideur a RÉELLEMENT décidé. Elle ne remplace pas
        # la première, elle la complète ; publier les deux est la décision de l'auteur.
        ...
        # R11 : le compte de choix forcés, à côté des parts et non perdu dans les compteurs.
        "choix_forces": {
            "n": n_forces,
...
def synthese_html(s: dict) -> str:
    ...
    for mode, pct in (s.get("parts_modales") or {}).get("pourcent", {}).items():
        ...
        # Les DEUX lectures côte à côte (R10) : toutes décisions, et hors itinéraire unique.
        # Un seul chiffre mélangerait ce que le décideur a décidé et ce que la situation lui
        # imposait, et la seconde quantité varie d'un bras à l'autre.
...
def perimetre_commun(traces_par_bras: dict[str, list[dict]]) -> dict:
    """L'intersection des déplacements que TOUS les bras ont réellement décidés (R12).

    Ni « toutes décisions » ni « hors itinéraire unique » ne suffisent pour comparer deux bras
    terme à terme. Le premier mélange ce qui a été décidé et ce qui était imposé. Le second
    retire des trajets **différents** selon le bras — 393 pour `duree_minimale`, 286 pour
    `aleatoire` sur le même substrat — donc il ne rend pas la comparaison plus juste, il la
    déplace : les deux colonnes cessent de porter sur le même ensemble.

    On prend donc l'intersection. Un déplacement y entre si, chez **chaque** bras, il est
    présent ET n'est pas un choix à itinéraire unique. Le résultat déclare son effectif et ce
    que chaque bras y perd : un périmètre restreint sans son motif se lit comme une donnée
    manquante.
    """
...
def comparer(a: str | Path, b: str | Path, *, inclure_invalides: bool = False) -> dict:
    ...
    return {
        ...
        "a": {
            ...
            # Les deux lectures voyagent avec la comparaison : celui qui la copie doit voir
            # que les parts mélangent décisions et contraintes, et de combien (R10, R11).
...
def formater_comparaison(c: dict) -> str:
    ...
    # Le rappel qui manque le plus quand on copie deux colonnes côte à côte (R12).
    out.append(
