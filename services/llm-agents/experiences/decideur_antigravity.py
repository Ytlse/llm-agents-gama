    ...
    def _verifier_journal_et_alarmes(self, now: float) -> None:
        ...
        if now - self._dernier_log_status >= 30.0:
            ...
            if self.compteurs["replis"] > 0 and not getattr(self, "_alarme_repli_levee", False):
                logger.error(
                    f"[ALARME] [antigravity] Replis uniformes détectés ({self.compteurs['replis']}) — "
                    "une expérience Antigravity doit maintenir strictement 0 replis."
                )
            ...
            while True:
                ...
                if reponse_path.is_file():
                    ...
                    if weights_are_fallback:
                        self.compteurs["rejets"] += 1
                        logger.error(
                            f"[ALARME] [antigravity] Réponse inexploitable pour {person.person_id}/{ctx.activity_id} "
                            f"— repli uniforme REFUSÉ en mode Antigravity. Format de 'probabilities' inattendu."
                        )
                        try:
