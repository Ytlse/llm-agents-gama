...
> **Étape 3a longitudinale (ticket 041, 2026-09-09).** Le dispositif à trois périodes (nominal / choc /
> rétabli, 5 à 60 jours) qui remplace la version « 5 jours » des fiches `exp_04a…d` est décrit dans
> [`ETAPE_3A_PLAN_LONGITUDINAL.md`](ETAPE_3A_PLAN_LONGITUDINAL.md) ; son ancrage bibliographique dans
> [`ETAPE_3A_LITTERATURE.md`](ETAPE_3A_LITTERATURE.md). Les fiches YAML correspondantes seront ajoutées
...
L'évaluation ne repose pas sur une facturation théorique au dollar, mais sur le respect des **quotas réels de débit et de plafonds journaliers du pool SWRR** configuré dans [`llm_module/config/providers.yaml`](../../../../config/llm_gateway/providers.yaml) et synthétisé dans [`docs/paper/methode/quotas_summary.html`](../quotas_summary.html).
...
|---|---|:---:|:---:|:---:|:---:|---|
| **`google_gemini31_key1`** | `gemini-3.1-flash-lite` | **15** | 250 000 | **500** | — | **Juge de référence** (`thinking: 0`, cache strict) |
| **`google_gemini35_key1`** | `gemini-3.5-flash-lite` | **15** | 250 000 | **500** | — | Mutateur / Variantes (+ thinking 1024) |
| **`google_gemini31_key2`** | `gemini-3.1-flash-lite` | **15** | 250 000 | **500** | — | Seconde clé Google (mesures hors campagne) |
...
3. **Seuil critique RPD (Pourquoi le batching est obligatoire)** :
   * **Sans batching** ($1\,000$ requêtes pour $1\,000$ trajets) : le run sature immédiatement le quota de 500 RPD de `google_gemini31_key1` à 50 % du jeu de données et échoue.
...
| **`exp_01a` Gemini Flash-Lite** | `gemini-3.1-flash-lite` | 1 000 | **100** | **20 %** (100 / 500 RPD) | Nul (15 RPM géré par SWRR) |
...
| **`exp_02a` Gemini Calibré** | `gemini-3.1-flash-lite` | 1 000 | **100** | **20 %** (100 / 500 RPD) | Nul (cache optimisé) |
...
Le fichier `experiments.yaml` couvre la totalité du plan de recherche AAMAS 2027. Les
identifiants ci-dessous sont ceux du **papier** (ils portent la phase et l'ordre des sections) ;
sur le disque, chaque expérience vit sous le nom que la plateforme **calcule** de ses paramètres,
donné par la clé `nom_runtime` de `experiments.yaml` (par exemple `exp_00a_random_otp` →
`exp_alea_nosim`, `exp_01a_bare_gemini_flash_lite` → `exp_gemini-31-fl_minper_jtir_t0_nosim`).
C'est ce nom-là que prennent `make experience-lancer EXP=…` et `data/experiences/<nom>/`
