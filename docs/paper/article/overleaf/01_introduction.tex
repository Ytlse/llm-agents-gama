% =====================================================================
% overleaf/01_introduction.tex — LaTeX rendering of en/01_introduction.md v0.25 (17 September 2026)
...
The study is conducted on the Toulouse metropolitan area, against the 2023 Cerema-certified household travel survey (EMC\textsuperscript{2} 2023) \citep{tisseo2023emc2}, about 16,000 respondents over 453 municipalities. % source: rapport AUAT/CEREMA 2023 ; BIBLIOGRAPHIE.md §5

\paragraph{C1. An evaluation contract on a common set of inputs.}
The contract has three rules. Each closes one way in which a comparison between a generative agent and a tabular model can be biased.

\begin{enumerate}
  \item \emph{A common set of inputs.} The LLM agent and the four tabular references fitted on the survey microdata, among them the multinomial logit and a LightGBM model \citep{ke2017lightgbm}, receive the same 21 input variables. % source: scripts/progedo_logit/feature_spec.json
  What is equalised is the inputs, not the exposure: the tabular references have read 39,203 survey trips % source: scripts/progedo_logit/mode_choice_policy_metrics.json, training.n_fit 31,279 + training.n_valid 7,924; the sum is the right count for exposure, early stopping (early_stopping_rounds 50, best_iteration 1500) making the validation rows bear on the model retained
  and the LLM has read none. At Level~2, the prompt carries behavioural guidance calibrated by hand against the survey's published aggregate shares, never against an individual record.
  \item \emph{Like-for-like readings.} Following the finding of \citet{meister2024benchmarking} that verbalised distributions align better than sampled ones, the agent is asked to spread 100\,\% over the itineraries presented, up to six per trip, rather than to pick one. % source: experience.yaml of the played experiments, max_candidats: 6
...
\begin{itemize}
  ...
  \item \emph{Level~3}: the four tabular references fitted on the survey, multinomial logit, gradient boosting (LightGBM), kernel logistic regression and random forest.
\end{itemize}

Several LLMs are evaluated, all at temperature~0.

\textbf{H0, distributional ceiling.} Neither the minimal prompt nor the expert prompt brings the LLM to the error of the best tabular reference on the four-mode split. No equivalence margin was written before the measurement, so Section~6 reports H0 as a paired estimate on the same personas: the sign, the amplitude and the interval of the gap between each LLM condition and each tabular reference, by cluster resampling at the person level. % source: § 6.4, which measures these paired differences; abstract v0.6, recommendation (a) of ticket 080
...
\begin{itemize}
  ...
  \item \emph{Five real, dated local news events.} Five real, dated articles of the Toulouse local press, chosen before any model call, each act through a channel that no tabular variable represents: % source: RAPPORT_SCENARIOS_QUALITATIFS_TOULOUSE.md §2 and §3
...
\begin{itemize}
  ...
  \item Section~4 defines the macro and micro metrics, the three rules of the contract, and the demographic control of the sealed cohort.
  \item Section~5 evaluates behaviour under the minimal prompt and describes the engineering of the expert prompt.
  \item Section~6 confronts the thirteen decision-makers with the survey, measures what prompt engineering moves and on which model, and reports H0.
  \item Section~7 presents the two non-tabulated regimes, hysteresis and news events, under the pre-registered directional predictions.
