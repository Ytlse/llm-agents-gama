...
%   ch6_audit_distance.png (sources in docs/paper/article/images/).
...
\subsection{Agreement between deciders, trip by trip}
\label{sec:agreement}
...
\begin{table}[tbp]
  ...
  \begin{tabularx}{\columnwidth}{@{}Lrr@{}}
    \toprule
    Pair & \multicolumn{1}{c}{Most likely} & \multicolumn{1}{c}{Drawn} \\
         & \multicolumn{1}{c}{mode} & \multicolumn{1}{c}{mode} \\
    \midrule
...
\begin{figure*}[tp]
  \centering
  \includegraphics[width=\textwidth]{ch6_audit_modes.png}
...
\begin{figure*}[tp]
  \centering
  \includegraphics[width=\textwidth]{ch6_audit_distance.png}
  \caption{Unit agreement by distance band. The deciders separate below 2 km, a third of the sample; beyond 10 km the all-car baseline matches the tabular ceiling.}
  \label{fig:audit-distance}
  \Description{A line chart. The horizontal axis carries six distance bands, from zero to one kilometre up to twenty to fifty kilometres, with the number of trips under each label. The vertical axis carries the agreement with the declared mode, in per cent, from thirty to ninety-five. Four series are plotted: gradient boosting, the expert prompt, the minimal prompt and the all-car baseline. On the first band the four series are widely spread, from thirty-seven per cent for the all-car baseline to sixty-two for gradient boosting. The spread narrows band by band, and on the last two bands the four series lie within two points of each other, above eighty per cent.}
\end{figure*}
...
%   and ch6_audit_distance are all figure*, chapter 6 carries three or four pages of near-solid
%   figures. Demoting ch6_audit_modes or ch6_audit_distance to single column, or moving one plate
