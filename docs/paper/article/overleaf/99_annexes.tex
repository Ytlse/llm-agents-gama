...
%   app:variables — Appendix E, the 21 variables. Referenced by overleaf/04_metrics_and_substrate.tex.
...
%   app:detail    — Appendix H, the detailed results. Referenced by overleaf/06_results.tex.
% THREE THINGS TO TAKE UP IN APPENDICES A TO G before submission, inherited from the French draft and NOT
%   repaired by the rendering: the "Tier 1 / 2 / 3" vocabulary; the figures, to be cross-checked from the
%   repository rather than copied from here; the cross-references, which followed the old manuscript numbering
%   (rewritten as \ref here, still hard-numbered in the Markdown master). Appendix H carries none of these.
% Preamble additions:
...
% Tables: nine, caption ABOVE, booktabs. The wide ones (variables, corrections, paired differences, error by
...
\section{Dictionary of the 21 variables of the evaluation contract}
\label{app:variables}

The parity contract of Section~\ref{sec:contract} designates the 21 variables of Table~\ref{tab:variables}, and them alone. The list is fixed in the repository (\texttt{spec\_version 2}) and it is that same file that builds the design matrix of the four tabular methods: neither the logit, nor gradient boosting, nor the random forest, nor the kernel regression sees anything else. % source: scripts/progedo_logit/feature_spec.json

\begin{table*}[t]
  \caption{The 21 variables of the evaluation contract, in three blocks. Every decider of Section~\ref{sec:levels} receives these, and the tabular references receive nothing else.}
  \label{tab:variables}
...
The inner centre is the centroid of the fine zones of the Capitole sector, and the distances are computed in the legal projection. A 19-variable variant, without the two distances to the inner centre, has been measured, accuracy \todo{xx} and composite \todo{xx}, and two alternative definitions of distance were set aside; the contract served is the 21-variable one. What the agent receives in addition to these 21 variables, the agenda of its remaining trips, the weather of the coming time slots and the itineraries themselves, is described in Section~\ref{sec:contract}.
...
\begin{table*}[t]
  ...
  \begin{tabular}{rp{0.42\textwidth}p{0.42\textwidth}}
    ...
    10 & Perimeter of the unit audit, 1{,}000 against 13{,}045 trips & Perimeters made explicit: tabular references on 13{,}045, LLM on a subsample of 1{,}000 drawn from the same set \\
...
% source: scores.json of the same runs as Section~\ref{sec:references}, composite.emd_jsd_tire; corrected set of ticket 088, runs of 2026-09-16.
...
\begin{table*}[t]
  \caption{The fourteen paired differences, on the three readings of the contract. Bold marks an interval that excludes zero.}
  \label{tab:paired}
  \begin{tabular}{lccc}
    \toprule
...
\begin{figure*}[t]
  \centering
  \includegraphics[width=\textwidth]{ch99_dimensions_voiture.png}
...
\begin{figure*}[t]
  \centering
  \includegraphics[width=\textwidth]{ch99_modes_distance.png}
...
\begin{figure*}[t]
  \centering
  \includegraphics[width=\textwidth]{ch99_modes_occupation.png}
...
\begin{figure*}[t]
  \centering
  \includegraphics[width=\textwidth]{ch99_modes_motif.png}
  \caption{The four modes by trip purpose. Education is the purpose where the agent moves furthest from the target after tuning, and it moves away in the direction of the car: the lever of the cost of engaging a vehicle acts on a population that has none.}
...
\begin{table}[t]
  ...
  \begin{tabular}{lrr}
    \toprule
    Stratum and model & Minimal & Expert \\
    \midrule
...
\begin{table}[t]
  \caption{The prompt variants measured on \texttt{gemini-3.5-flash-lite}. The two variants tuned in sight of the evaluated cohort were meant to give an upper bound, having seen the residuals on which they are then judged; they do worse than the variant tuned out of sample.}
  \label{tab:variants}
  \begin{tabular}{llrrr}
    \toprule
    ...
    \texttt{prompt\_expert\_05}  & tuned out of sample   & 4.86 &  6.86 & 13.85 \\
...
What the protocol foresaw as an in-sample fitting ceiling is not reached by the prompts that had access to it. % source: data/experiences/exp_gemini-35-fl_{promin02,proexp05,proexp06,proexp08}_...; prompt_expert_07 declared, not played; prompt_expert_04 deleted from the repository on 2026-09-17
