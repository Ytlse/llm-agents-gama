...
**Version du document :** `v1.4` (2 septembre 2026) — *Mise en cohérence des chiffres publiés avec les mesures du dépôt (règle argmax, contrat à 21 variables, requalification du jalon 0) et renforcement de la réfutabilité de l'étape 3 (bras paraphrase, placebo, oracle informé, pré-enregistration)*  
**Fichiers associés :** [`PROTOCOLE_SCIENTIFIQUE.md`](PROTOCOLE_SCIENTIFIQUE.md), [`PLAN_ARTICLE_2026.md`](PLAN_ARTICLE_2026.md), [`SLIDES_SEMINAIRE_2026_v1.0.html`](SLIDES_SEMINAIRE_2026_v1.0.html)  
...
* `v1.4` *(2 septembre 2026)* : Audit de cohérence entre le manuscrit et les mesures du dépôt — onze corrections listées en **Annexe F**. Les principales : comparaison L1 ramenée à l'argmax des deux côtés, parité informationnelle portée à 21 variables, jalon 0 requalifié en contrôle de cohérence, dissymétrie d'exposition aux données déclarée, et protocole de l'étape 3b porté à cinq conditions (paraphrase sans indice modal, article placebo, oracle recevant l'événement encodé).
...
1. **Pas de LLM pour la prédiction de masse statique** : Les arbres supervisés (LightGBM) sont $\approx 2\,700\times$ plus rapides (moins d'une seconde contre $\approx 45$ minutes pour $10\,000$ trajets), gratuits en tokens et quatre fois plus fidèles en argmax.
2. **La valeur du LLM réside dans la dépendance** : non tabulée (presse locale, dégoût, densité de foule, agrément) et non indépendante (hystérésis $J+1$, arbitrage intra-ménage, chaîne spatiale des véhicules). Partout ailleurs, un arbre de décision fait mieux et moins cher.
4. **Un enseignement de mesure, transférable hors de ce cas** : une variante à $93,4\,\%$ d'accuracy sur l'enquête a produit le **pire** score en simulation ($9,28$ contre $7,40$ de composite), la distance reconstruite depuis la durée déclarée contenant le mode retenu. Toute évaluation d'agent génératif doit être notée **là où le modèle sert**, pas là où il est facile de le noter.
