...
**Document:** chapter 6 of the AAMAS 2027 paper — the **results** chapter: chapter 5 defines the conditions compared, this one says what the comparison establishes. English master; French mirror in [`fr/06_Empirical_Evaluation.md`](../fr/06_Empirical_Evaluation.md); LaTeX rendering for Overleaf in [`overleaf/chapters/06_Empirical_Evaluation.tex`](../overleaf/chapters/06_Empirical_Evaluation.tex).
**Status:** `draft v0.5` (22 September 2026) — the section on agreement between deciders is removed at the author's request: its three-pair table, its reading paragraph and its source comment go with it. The two following sections move up one rank, the unit audit becoming **§ 6.4** and cohort variation **§ 6.5**. Five sections, six figures, numbered independently of the sections. Mirror of the French `brouillon v0.9`. No figure of the remaining sections changes.
**Previous status:** `draft v0.4` (17 September 2026) — first English master, translated from the French `brouillon v0.4` of the same day. The French carries the version history; it is not repeated here. Five sections, six figures already composed in English: four by `scripts/analysis/plot_chapitre6.py`, the two of the unit audit by `scripts/analysis/plot_audit_unitaire.py`; the detail tables live in appendices H and I.
