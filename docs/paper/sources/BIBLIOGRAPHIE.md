...
* **Bin Tareaf, R. et al. (2026)** — *Benchmarking large language model agent societies against human behavioural distributions (SILICA)*. arXiv / Open-Source Benchmark (`raadbintareaf/silica-benchmark`).
  ...
  * **Lien avec notre article :** Fournit la justification théorique formelle de notre constat à l'Étape 2 : les agents sous prompt factuel neutre et circonstancié ou calibré par prompt échouent systématiquement face aux distributions empiriques de l'enquête EMC² 2023 ($29,81\text{ pt}$ d'erreur L1 vs $7,30\text{ pt}$ pour l'oracle LightGBM). Cet échec n'est pas un défaut de prompt engineering local, mais un **plafond structurel de Tier 3** inhérent aux priors pré-entraînés mondiaux des modèles de fondation.
