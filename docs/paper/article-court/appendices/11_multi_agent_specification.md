...
## 11.1 Formal Agent Tuple and Physical Vehicle Chain Constraints
...
## 11.2 Intellectual Filiation and the Four Architectural Divergences
...
### Divergence 1: Reinstatement of Event Gravity
...
### Divergence 2: Triple Additive Consolidation Triggers
...
Our architecture unifies both principles through three additive triggering rules shown in Table 11.1.

*Table 11.1. Additive triggering mechanisms governing cognitive reflection.*

| Trigger | Formal condition | Frequency / Regime |
|---|---|---|
...
### Divergence 3: Retrieval-Based Decay and Calibrated Half-Life
...
### Divergence 4: Elimination of Min-Max Normalization
...
## 11.3 Five-Phase Nycthemeral Lifecycle and Multi-Agent Synchronization

Agents navigate a synchronized five-phase daily cognitive cycle. Figure 11.1 illustrates the six operational processing stages linking physical events to prompt generation.

![Complete Multi-Agent Cognitive Memory Architecture and Lifecycle](images/fig_memory_lifecycle.png)
*Figure 11.1. Processing stages of the multi-agent cognitive memory pipeline, from sensory perception to prompt injection.*

The nycthemeral cycle organizes these stages across distinct temporal windows summarized in Table 11.2.

*Table 11.2. Five operational phases of the synchronized multi-agent daily cycle.*

| Phase | Time window | Operational activities |
|---|---|---|
| Phase 1: Waking & Planning | 05:00 to 09:00 | Morning commute evaluation, retrieval of consolidated concepts |
| Phase 2: In-Trip Execution | 09:00 to 18:00 | Physical stage execution, transfer logging, short-term buffer updates |
| Phase 3: Household Exchange | 18:00 to 22:00 | Dinner conversation, 1-hop belief sharing with family members |
| Phase 4: Evening Floor | 22:00 | Structured LLM reflection, concept updating, EDF queue dispatch |
| Phase 5: Nighttime Drainage | 22:00 to 05:00 | Background reflection completion during physical simulation lull |
...
## 11.4 Multi-Pool Candidate Generation and Five-Component Retrieval Scoring

To balance semantic nuance and physical relevance, our retrieval engine draws candidate memories from three complementary pools shown in Table 11.3.

*Table 11.3. Multi-pool candidate memory generation architecture.*

| Pool | Storage backend | Query logic | Quota |
|---|---|---|---|
...
The weather affinity contributes a binary indicator matching atmospheric conditions. Figure 11.2 illustrates the resulting retrieval profiles across three representative mobility situations.

![Five-Component Episodic Memory Retrieval Scoring Profile](images/fig_retrieval_components.png)
*Figure 11.2. Radar chart of the five retrieval scoring components across routine, breakdown, and weather shock profiles.*

## 11.5 Cognitive Decay Dynamics and Recall Reinforcement
...
Figure 11.3 contrasts our calibrated mobility decay against the classical Park et al. baseline and demonstrates active recall reinforcement.

![Cognitive Memory Retention: Ebbinghaus Decay and Recall Reinforcement](images/fig_ebbinghaus_decay.png)
*Figure 11.3. Retention trajectories showing rapid decay for routine commutes, persistent retention for traumatic shocks, and recall reinforcement.*
...
## 11.6 Concept Evolution, Laplace Confidence, and Dual Extinction Mechanics
...
During consolidation, the model inspects existing active beliefs and applies one of the four structured mutations listed in Table 11.4.

*Table 11.4. Four structured operations governing conceptual belief evolution.*

| Operation | Action on existing concept | Effect on confidence |
|---|---|---|
| `creer` | Instantiates a novel concept | Initial prior ($\text{obs}=0, \text{contre}=0, \text{Conf}=0.50$) |
| `confirmer` | Confirms current travel rule | Increments observation count ($\text{obs} \leftarrow \text{obs} + 1$) |
| `preciser` | Updates descriptive scope | Preserves empirical counters and confidence |
| `contredire` | Records counter-example | Increments counter-examples ($\text{contre} \leftarrow \text{contre} + 1$) |
...
## 11.7 Working Context Formulation and Prompt Injection

Decisions do not feed raw unstructured memory streams to the language model. Instead, the simulation controller compiles memory states into three deterministic context blocks. Table 11.5 outlines their structure and operational sources.

*Table 11.5. Structure of the three deterministic working context blocks.*

| Context block | Generation source | Injected content |
|---|---|---|
| `My habits` | Arrival trip logs | Empirical mode frequencies and delay counts |
| `What I know` | Consolidated concepts | High-confidence travel beliefs ($\text{Conf} \ge 0.50$) |
| `What changed recently` | Press and shock logs | Disruptions, de-indexed beliefs, press bulletins |
