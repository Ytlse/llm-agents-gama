...
Pourquoi cette frontière. Auparavant, l'import suffisait. Un script d'analyse, un contrôle de configuration ou un simple shell dans un conteneur faisait donc basculer `experiments/current` vers un répertoire vide, et tout ce qui résout ce lien **à chaque écriture** — le journal des échanges avec le modèle — se mettait à écrire à côté du run en cours. Quatre occurrences entre le 2026-09-03 au soir et le 2026-09-04 au matin, la première ayant détourné **1 037 échanges** en une minute ; les répertoires parasites ont dû être renommés à la main. Le garde-fou précédent (pytest, `unittest`, `APP_NO_RUN_ARTIFACTS=1`) ne couvrait que les suites de tests, pas un script lancé à la main : il reste en place, et neutralise aussi `claim_run()`.

Conséquence pratique : un script qui a besoin du répertoire du run ne doit pas l'attendre de l'import. Il lit `experiments/current` (le run réellement en cours) ou reçoit son chemin en argument.
