...
| `synthesis/population/enriched.py` (`_assign_personal_bike`) | `personal_bike` | **ticket 015, lot 4** : les trois étages appris sur EMC² 2023 remplacent l'imputation Bernoulli. `k` tiré **par ménage** (`household_id`, nativement présent) conditionnellement à la zone fine du domicile / la taille / la motorisation, attribué par tirage sans remise pondéré par la propension `P20`, puis VAE à 7,7 % **du parc** (et non 14,8 % des porteurs). Déterministe par hachage — ne consomme plus `random_seed`. Nouvelle dépendance de stage : `spatial.home.locations` (pour la zone). **RETIRÉ le 2026-09-04** (ticket 034, lot 2) : le lot 4 avait été rejeté le 2026-08-24, mais le code tournait encore et contredisait l'étage D pour 4,7 % du vivier, faute de clés communes. Ce stage n'écrit plus de colonne `personal_bike`, et l'export JSON n'invente plus « Pas de vélo » à sa place — le trait vient de l'étage D, seul. Mesuré avant retrait : 14 personas concernés (tous sans domicile, tous exclus par la sélection), cohorte scellée identique au bit près. Détail : [velo-equipement.md](velo-equipement.md) |
...
| Ressource | `packages/mobility_core/src/mobility_core/data/pt_subscription.json` | `packages/mobility_core/src/mobility_core/data/driving_license.json` |
...
`packages/mobility_core/src/mobility_core/equipment_propensity.design_vector()` est la **seule** définition, et
...
services/llm-agents/.venv/bin/python -m scripts.data.population.enrich_equipment /tmp/pop_cf.json
services/llm-agents/.venv/bin/python -m scripts.data.population.fix_minor_traits  /tmp/pop_cf.json
```
...
([`_can_drive`, vehicle_chain.py](../../llm-agents/urban_mobility_agents/vehicle_chain.py), importé par `simulation_controller.py`).
