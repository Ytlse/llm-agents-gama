...
| Timestamp | Relevé | Description |
|-----------|--------|-------------|
| `T_extract_end` | Après tirage du mode dans les `probabilities` + remapping | `T_extract_end - T_llm_result` ≈ durée de normalisation du vecteur de probabilités, du tirage et du remapping vers l'index original (post-shuffle). |
